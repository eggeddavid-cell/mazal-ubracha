...
# ─── DESK ENGINE HTML (bat_sheva 10 engines, light mode) ──────────────────────
import os as _os
_DESK_HTML_PATH = _os.path.join(_os.path.dirname(__file__), "desk_engine.html")
def _get_desk_html():
    if _os.path.exists(_DESK_HTML_PATH):
        with open(_DESK_HTML_PATH, "r", encoding="utf-8") as _f:
            return _f.read()
    return "<p>desk_engine.html לא נמצא בתיקייה</p>"
...
# ─── ANALYSIS MODE ────────────────────────────────────────────────────────────
    ...
    # ── TABS ──────────────────────────────────────────────────────────────────
    tab_chart, tab_mtf, tab_flow, tab_news, tab_bt, tab_risk, tab_ind, tab_checks, tab_desk = st.tabs([
        "📊 גרף", "🕐 Multi-TF", "🔄 Flow Analysis",
        "📰 חדשות", "🧪 Backtest", "⚖️ סיכון",
        "📐 אינדיקטורים", "🏆 ניקוד", "🎯 דסק מסחר"
    ])
    ...
    with tab_desk:
        # Pass current ticker + price to the engine via URL params
        tickers_param = ",".join([symbol] + [x.strip().upper() for x in watchlist_text.split(",") if x.strip()])
        prices_param  = f"{symbol}:{last_p:.2f}"
        desk_html = _get_desk_html()
        # Inject params into the HTML before loading
        desk_html = desk_html.replace(
            "urlP.get('tickers') || 'SPY,QQQ,AAPL,NVDA,TSLA,MSFT,META,AMZN,AMD,GOOGL'",
            f"'{tickers_param}'"
        ).replace(
            "urlP.get('prices')  || ''",
            f"'{prices_param}'"
        )
        st.components.v1.html(desk_html, height=1800, scrolling=True)
