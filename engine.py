...
def analyze_symbol(symbol, df, levels, patterns, tf_data, options, horizon, account_size, risk_pct):
    ...
    for name, tdf in tf_data.items():
        ...
        call_prob += 2.5 if d=="CALL" else -2.5 if d=="PUT" else 0

    text = " ".join([p.get("כיוון","") for p in patterns])
    if "שורית" in text: call_prob += 2
    if "דובית" in text: call_prob -= 2

    # Options layer
    if options and options.get("source") != "none":
        pcr = options.get("put_call_oi_ratio")
        if pcr is not None:
            if pcr < 0.85:
                call_prob += 5; checks.append({"בדיקה":"Put/Call OI שורי","תוצאה":"חיובי","משקל":5})
            elif pcr > 1.15:
                call_prob -= 5; checks.append({"בדיקה":"Put/Call OI דובי","תוצאה":"שלילי","משקל":5})
        mp = options.get("max_pain")
        if mp:
            if float(last["Close"]) > mp:
                call_prob += 3; checks.append({"בדיקה":"מחיר מעל Max Pain","תוצאה":"חיובי","משקל":3})
            else:
                call_prob -= 3; checks.append({"בדיקה":"מחיר מתחת Max Pain","תוצאה":"שלילי","משקל":3})
        gex = str(options.get("gex_proxy",""))
        if "חיובי" in gex:
            call_prob += 3; checks.append({"בדיקה":"GEX Proxy חיובי","תוצאה":"חיובי","משקל":3})
        elif "שלילי" in gex:
            call_prob -= 3; checks.append({"בדיקה":"GEX Proxy שלילי","תוצאה":"שלילי","משקל":3})
    ...
    confidence = int(max(35, min(95, 45 + min(20, sample*0.4) + (10 if abs(call_prob-50)>=15 else 0) + (10 if options.get("source")!="none" else 0))))
    ...
    summary = f"{symbol}: {action}, Grade {grade}, CALL {call_prob}%, PUT {put_prob}%, אמינות {confidence}%. Max Pain: {options.get('max_pain')}. Call Wall: {options.get('call_wall')}. Put Wall: {options.get('put_wall')}."
