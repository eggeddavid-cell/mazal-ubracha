...
def create_pdf_report(symbol, report, levels, options):
    buffer = BytesIO()
    ...
    if options:
        lines += [
            "",
            "Options:",
            f"Call Volume: {options.get('call_volume')}",
            f"Put Volume: {options.get('put_volume')}",
            f"Put/Call Volume Ratio: {options.get('put_call_volume_ratio')}",
        ]
