...
def create_pdf_report(symbol, result, levels, patterns, options):
    b = BytesIO(); c = canvas.Canvas(b, pagesize=A4); w,h = A4; y = h-50
    c.setFont("Helvetica-Bold", 18); c.drawRightString(w-40, y, f"Options Daily Trend AI v9 - {symbol}"); y -= 35
    c.setFont("Helvetica", 12)
    lines = [
        f"Action: {result['action']}", f"Grade: {result['grade']}",
        f"CALL: {result['call_prob']}% | PUT: {result['put_prob']}% | WAIT: {result['wait_prob']}%",
        f"Confidence: {result['confidence']}%", f"Last Close: {result['last_close']:.2f}",
