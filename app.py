...
st.markdown('<div class="big-title">Options Daily Trend AI v4</div>', unsafe_allow_html=True)
st.markdown('<div class="small-muted">Cache + טיפול בחסימות Yahoo + זיהוי תבניות נרות + CALL/PUT/WAIT</div>', unsafe_allow_html=True)
...
with st.sidebar:
    ...
    st.caption("מומלץ להתחיל עם 5d ו־15m כדי להפחית חסימות Yahoo.")
...
if run and symbol:
    with st.spinner("מושך נתונים ומחשב..."):
        ...
        if data_error:
            ...
            st.stop()
        if df.empty:
            st.warning("לא התקבלו נתונים. נסה 5d / 15m או סימול אחר.")
            st.stop()

        df = add_indicators(df)
        if len(df) < 25:
            st.warning("אין מספיק נתונים לחישוב איכותי. נסה תקופה ארוכה יותר או טיים פריים אחר.")
            st.stop()
        ...
        report = build_report(symbol, df, levels, options, news_items, patterns)
    ...
    with c2:
        st.markdown(f'<div class="score-box"><div>ציון הסתברות</div><div class="score-number">{report["score"]}%</div></div>', unsafe_allow_html=True)
    with c3:
    ...
    pdf_bytes = create_pdf_report(symbol, report, levels, options, patterns)
    st.download_button("הפק דוח PDF", data=pdf_bytes, file_name=f"{symbol}_daily_report.pdf", mime="application/pdf", use_container_width=True)

    tab5, tab4, tab3, tab2, tab1 = st.tabs(["חדשות", "אופציות", "תבניות נרות", "בדיקות", "גרף מקצועי"])
    ...
    with tab4:
        if options:
            ...
            st.warning("נתוני אופציות לא זמינים כרגע.")
    with tab5:
        if news_items:
